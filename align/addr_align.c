...
#define LITTLEENDIAN
#ifdef LITTLEENDIAN
#define DATA8(name) ((src << 8) | (src >> 24));
#define DATA16(name) ((src << 16) | (src >> 16));
#else
#define DATA8(name) ((src >> 8) | (src << 24))
#define DATA8(name) ((src >> 16) | (src << 16))
#endif
...
{										    \
	...
	if ((int)src_line & 0x03){							    \
		if (len >= 4){								    \
			 ...
			 */								    \
			if (((int)src_line & 0x01) && ((int)src_line & 0x02)){						    \
			...
			if (((int)src_line & 0x01)){						    \
				...
				src_line    =   (void*)(((int)(src_line)) + 1);			    \
				len	    -=	1;						    \
				tmp	    =   tmp  | (*((uint8_t*)(src_line))<<8);	    \
				src_line    =   (void*)(((int)(src_line)) + 1);			    \
				tmp	    =   tmp  | (*((uint8_t*)(src_line))<<16);	    \
				src_line    =   (void*)(((int)(src_line)) + 1);			    \
				shift	    +=	16;						    \
				len	    -=	2;						    \
				printf("1\n");					\
...
\
\
static void tiled_rop_ ## _name(void* ptr, uint32_t len, void* tile, void* tile_end, uint32_t tile_width)   \
{													    \
	int		shift;											    \
	uint32_t	concat;											    \
	void*	tile_tail;										    \
	...
	typedef void (*solid_rop_func_t)(void* ptr, uint32_t val, int len);
	typedef void (*copy_rop_func_t)(void* ptr, void* src, int len);
	typedef void (*tiled_rop_func_t)(void* ptr, int len, void* tile, void* tile_end, int tile_width);

#define ROP_TABLE(_type, _size)                                                 \
		static void (*solid_rops[16]) (void* ptr, uint32_t val, int len) = {		\
			solid_rop_clear,								\
			solid_rop_and,								\
	...
	static void (*copy_rops[16]) (void* ptr, void* src, int len) = {		\
		copy_rop_clear,								\
		copy_rop_and,								\
	...
	static void (*tile_rops[16]) (void* ptr, int len, void* tile, void* tile_end, int tile_width) = {	    \
		tile_rop_clear,								\
		tile_rop_and,								\
...
int main(int argc, char** argv)
{
	...
	printf("src+1: %p\n", (src+1));
	solid_rop_copy((uint8_t*)(src+1), data, 4*1024-1);
	printf("solid done\n");
	for (i=0; i<32; i++){
	...
	copy_rop_copy((uint8_t*)dst+1, src, 4*1024);
	printf("copy done\n");
	...
	tiled_rop_copy((uint8_t*)dst+3, 4*1024, src, src+4*1024, 1024);
	printf("tile done\n");
